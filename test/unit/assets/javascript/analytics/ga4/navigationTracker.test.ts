...
describe("NavigationTracker", () => {
  ...
  afterEach(() => {
    ...
    delete require.cache[
      require.resolve(
        "../../../../../../src/assets/javascript/analytics/ga4/navigationTracker.js",
      )
    ];
  });
  ...
  describe("trackButtonClick", () => {
    describe("Tracking information not configured", () => {
      it("Does not send navigation event", () => {
        global.window.DI.core.trackerFunctions.findTrackingAttributes = () =>
          null;
        new global.window.DI.analyticsGa4.trackers.NavigationTracker(
          navigationElement,
        ).init();
        ...
        expect(consoleWarnSpy).to.be.calledWith(
          "Could not find the navigation tracker configuration",
        );
      });
    });
    ...
    describe("Invalid tracker configuration", () => {
      ...
      describe("Missing configuration property", () => {
        it("Does not send navigation event", () => {
          ...
          expect(mockSendData).not.to.have.been.called;
          expect(consoleWarnSpy).to.be.calledWith(
            "Missing ga4-data-navigation configuration properties. Expected: [text: string, type: string, external: string]",
          );
        });
      });
      ...
      describe('Text property is not an instance of "string"', () => {
        it("Does not send navigation event", () => {
          navigationElement.setAttribute(
            "ga4-data-navigation",
            '{ "text": [], "type": "type", "external": "external" }',
          );
          new global.window.DI.analyticsGa4.trackers.NavigationTracker(
            navigationElement,
          ).init();
    ...
    describe("Button click event", () => {
      it("Sends navigation event with button text and additional data attributes", () => {
        navigationElement.setAttribute(
          "ga4-data-navigation",
          '{ "text": "text", "type": "type", "external": "external" }',
        );
        new global.window.DI.analyticsGa4.trackers.NavigationTracker(
          navigationElement,
        ).init();
