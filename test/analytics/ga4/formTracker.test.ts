...
describe('FormTracker', () => {
  ...
  afterEach(() => {
    ...
    delete require.cache[require.resolve('../../../src/assets/javascript/analytics/ga4/formTracker')];
  });
        ...
        describe('Inputs defined outside of labels', () => {
          ...
          describe('Tracker configuration missing', () => {
            it('Sends undefined values for type, section and action', () => {
              ...
              expect(mockSendData).to.have.been.calledWith({
                event: 'event_data',
                event_data: new EventDataBuilder().withText('r1 label').withType(undefined).withSection(undefined).withAction(undefined).build()
              })
            })
          })
        })
